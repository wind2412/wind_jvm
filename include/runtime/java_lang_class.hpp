...
public:
	...
	static unordered_map<wstring, Oop*> & get_single_basic_type_mirrors() {
		static unordered_map<wstring, Oop*> basic_type_mirrors;		// a map to restore basic type mirrors. Such as `int`, `long`...。 `int[]`'s mirror will be `int`'s.
	...
	static Oop *get_basic_type_mirror(const wstring & signature) {	// "I", "Z", "D", "J" ......	// must execute this after `fixup_mirrors()` called !!
		...
		unordered_map<wstring, Oop*>::iterator iter;
