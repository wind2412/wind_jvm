...
//	thread.add_frame_and_execute(hashtable_put, {prop, java_lang_string::intern(L"java.vendor.url.bug"), java_lang_string::intern(L"http://bugreport.sun.com/bugreport/")});
	thread.add_frame_and_execute(hashtable_put, {prop, java_lang_string::intern(L"java.home"), java_lang_string::intern(L"/home")});				// 乱写的。不过不能不指定这个变量就好！因为 UnixFileSystem 中 canonicalize() 方法指明这个 java.home 不可以是 null，否则会崩溃。
