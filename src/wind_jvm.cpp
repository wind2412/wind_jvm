...
wind_jvm::wind_jvm(const wstring & main_class_name, const vector<wstring> & argv) : main_class_name(main_class_name), rsp(-1), pc(0) {
	...
	std::static_pointer_cast<ObjArrayKlass>(BootStrapClassLoader::get_bootstrap().loadClass(L"[Ljava/lang/String;"));			// 手动调用
	this->vm_stack.push_back(StackFrame(nullptr, main_method, nullptr, nullptr));		// TODO: 暂时设置 main 方法的 return_pc 和 prev 全是 nullptr。
}
